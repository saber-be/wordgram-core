...
class TestDailyCounterReset(unittest.TestCase):
    ...
    def test_daily_counter_reset_function_successfully(self):
        client = MongoClient()
        db = client['test_db']
        ...
        # Testing daily_counter_reset
        client_service.daily_counter_reset("test_shop", db)
        shop = db['clients'].find_one({"instagram_username": "test_shop"})
        self.assertEqual(shop['daily_instagram_sync_count'], 0)
        self.assertEqual(shop['daily_website_update_count'], 0)

        db["clients"].find_one_and_delete({"instagram_username": "test_shop"})

    @patch('pymongo.MongoClient', new=mongomock.MongoClient)
    @freeze_time("2024-01-02 23:00:00")
    def test_daily_counter_reset_function_failed(self):
        client = MongoClient()
        db = client['test_db']
        ...
        client_service.daily_counter_reset("test_shop", db)
        shop = db['clients'].find_one({"instagram_username": "test_shop"})
        self.assertEqual(shop['daily_instagram_sync_count'], 5)
        self.assertEqual(shop['daily_website_update_count'], 10)

        db["clients"].find_one_and_delete({"instagram_username": "test_shop"})

    @patch('pymongo.MongoClient', new=mongomock.MongoClient)
    @freeze_time("2024-01-02 23:00:00")
    def test_daily_counter_reset_function_unset_keys(self):
        client = MongoClient()
        db = client['test_db']
