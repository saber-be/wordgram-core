...
class TestWebSiteUpdateCounterIncrement(unittest.TestCase):
    @patch('pymongo.MongoClient', new=mongomock.MongoClient)
    @freeze_time("2024-01-02 12:00:00")
    def test_website_update_counter_increment_function_successfully(self):
        client = MongoClient()
        db = client['test_db']
        ...
        # Testing website_update_counter_increment
        client_service.website_update_counter_increment("test_shop", db)
        shop = db['clients'].find_one({"instagram_username": "test_shop"})
        self.assertEqual(shop['daily_website_update_count'], 1)
        self.assertEqual(shop['website_update_count'], 1)
        self.assertEqual(shop['last_website_update'], datetime.now())

        db["clients"].find_one_and_delete({"instagram_username": "test_shop"})

    @patch('pymongo.MongoClient', new=mongomock.MongoClient)
    @freeze_time("2024-01-02 23:00:00")
    def test_website_update_counter_increment_function_unset_keys(self):
        client = MongoClient()
        db = client['test_db']
