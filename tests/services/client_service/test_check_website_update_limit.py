...
class TestCheckWebSiteUpdateLimit(unittest.TestCase):
    @patch('pymongo.MongoClient', new=mongomock.MongoClient)
    @freeze_time("2024-01-02 12:00:00")
    def test_check_website_update_limit_function_daily_successfully(self):
        client = MongoClient()
        db = client['test_db']
        ...
        shop['daily_website_update_count'] = 8
        # Inserting data
        db['clients'].update_one({"instagram_username": "test_shop"}, {
                                 "$set": shop}, upsert=True)

        # Testing check_website_update_limit
        result = client_service.check_website_update_limit("test_shop", db)
    ...
    @patch('pymongo.MongoClient', new=mongomock.MongoClient)
    @freeze_time("2024-01-02 12:00:00")
    def test_check_website_update_limit_function_successfully(self):
        client = MongoClient()
        db = client['test_db']
    ...
    @patch('logging.error')
    def test_check_website_update_limit_function_daily_failed(self, mock_logger):
        client = MongoClient()
        db = client['test_db']
        ...
        # copy of default
        shop = default_shop_value.copy()
        shop['daily_website_update_count'] = 10
        # Inserting data
        db['clients'].update_one({"instagram_username": "test_shop"}, {
                                 "$set": shop}, upsert=True)
    ...
    @freeze_time("2024-01-02 23:00:00")
    @patch('logging.error')
    def test_check_website_update_limit_function_failed(self, mock_logging):
        client = MongoClient()
        db = client['test_db']
    ...
    @freeze_time("2024-01-02 23:00:00")
    @patch('logging.info')
    def test_check_website_update_limit_function_less_1_hour_failed(self, mock_logging):
        client = MongoClient()
        db = client['test_db']
        ...
        # copy of default
        shop = default_shop_value.copy()
        shop['last_website_update'] = datetime.strptime("2024-01-02 22:10:00", "%Y-%m-%d %H:%M:%S")
        # Inserting data
        db['clients'].update_one({"instagram_username": "test_shop"}, {
                                 "$set": shop}, upsert=True)
